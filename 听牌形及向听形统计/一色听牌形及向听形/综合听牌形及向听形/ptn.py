...
def findagari(tf = False, cf = False):
	agari = [[], [], [], []]
	...
	for i in range(4):
		...
		if cf:
			agari[i][14].extend(toitsuptn(7, a))
...
def findtenpai(agari):
	...
	# find tenpai pattern with sutehai
	...
	# find tenpai pattern with no sutehai
	tenpaiset = create_set_from(tenpai)
	for i in range(4):
		for j in range(14):
			for k in agari[i][j + 1]:
				for m in set(k):
					rk = k.copy()
					rk.remove(m)
					rk.sort()
					if hash(tompsz(rk)) not in tenpaiset:
						tenpai[i][j].append(rk)
						tenpaiset[i][j].add(hash(tompsz(rk)))
...
def main():
	...
	# normal tenpai pattern together with chitoitsu tenpai pattern
	with open("tenpaichi.json") as f:
		tenpaichi = json.load(f)
	tenpai = tenpaichi
	...
	# normal shanten1 pattern together with chitoitsu shanten1 pattern
	with open("shanten1chi.json") as f:
		[remain, shanten1chi] = json.load(f)
	shanten1 = shanten1chi
	...
	# normal shanten2 pattern together with chitoitsu shanten2 pattern
	with open("shanten2chi.json") as f:
		[remain, shanten2chi] = json.load(f)
	shanten2 = shanten2chi
	...
	# normal shanten3 pattern together with chitoitsu shanten3 pattern
	with open("shanten3chi.json") as f:
		[remain, shanten3chi] = json.load(f)
	shanten3 = shanten3chi
	...
	# normal shanten4 pattern together with chitoitsu shanten4 pattern
	with open("shanten4chi.json") as f:
		[remain, shanten4chi] = json.load(f)
	shanten4 = shanten4chi

	# # find normal shanten5 pattern
	# for i in range(4):
	# 	for j in range(15):
	# 		remain[i][j] = removeptn(remain[i][j], shanten4[i][j])
	# shanten5 = findshanten(remain, shanten4)
	# # printptn(shanten5)
	# # print([remain, shanten5])
	# with open("shanten5.json") as f:
	# 	[remain, shanten5] = json.load(f)

	# # normal shanten5 pattern together with toitsu shanten5 pattern
	# with open("shanten5toi.json") as f:
	# 	[remain, shanten5toi] = json.load(f)
	# shanten5 = shanten5toi

	# normal shanten5 pattern together with chitoitsu shanten5 pattern
	with open("shanten5chi.json") as f:
		[remain, shanten5chi] = json.load(f)
	shanten5 = shanten5chi

	# # combined code to find shanten[0~4] pattern at once (would also cost combined running time)
